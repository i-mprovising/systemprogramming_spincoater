...
void* ultrasonic() {
    ...
    while (1) {
        ...
        if (distance < LID_MIN || LID_MAX < distance) { // 뚜껑이 원래 위치에서 벗어남 -> 즉 뚜껑을 웨이퍼에 덮음
            count++;
            if (count > 6) {
                usleep(5000000);
                write(SOCK, "LID", sizeof("LID")); // 뚜껑이 덮힌 것을 서버에 알림.  
                printf("* Lid is on\n");
                return 0;
            }
        } else {
...
int move_wafer(char dir, int dist) {
    ...
    min_duty = PWM_PERIOD * SERVO_MIN_DUTY / 100;
    // dist 만큼 모터가 움직이고 다시 원래 자리로 위치
    if (dir == 'U') { 
        printf("* Move down \n\n");
        softPwmWrite(SERVO4, duty/100000);
        usleep(1000000);
        softPwmWrite(SERVO4, min_duty/100000);
    }
    else if (dir == 'D') {
        printf("* Move up \n\n");
        softPwmWrite(SERVO3, duty/100000);
        usleep(1000000);
        softPwmWrite(SERVO3, min_duty/100000);
        ...
        printf("* Move right \n\n");
        softPwmWrite(SERVO2, duty/100000);
        usleep(1000000);
        softPwmWrite(SERVO2, min_duty/100000);
    }
    else if (dir == 'R') {
        printf("* Move left \n\n");
        PWMWriteDutyCycle(SERVO1, duty);
        usleep(1000000);
        PWMWriteDutyCycle(SERVO1, min_duty);
    }
    usleep(1000000);
...
int main(int argc, char *argv[]) {
    ...
    while(1) {
		...
		if (str_len == -1) {
            ...
            if (strncmp(msg, "Stop", 4) * strncmp(msg, "Error", 5) * strncmp(msg, "End", 3) == 0) {
                // 종료 신호
                if (flag == 1){
